...
export default function OrdersTable({
  ...
  // ✅ shu bo‘lmasa setAssignOpen topilmaydi
  ...
  const sortBtn = (key: any, label: string) => {
    ...
    return (
      <button
        type="button"
        className={`inline-flex items-center gap-1 font-semibold ${active ? "text-slate-900" : "text-slate-600"}`}
        onClick={() => setSort({ key, dir: nextDir })}
      >
        {label}
        {active ? <span className="text-xs">{sort.dir === "asc" ? "▲" : "▼"}</span> : null}
      </button>
    );
  };
  ...
  return (
    <>
      <Card className="overflow-hidden">
        ...
        {/* <div className="px-4 py-3 md:px-5 md:py-4 border-b border-slate-200 flex items-center gap-2">
          <div>
            ...
            <div className="text-xs text-slate-500">Zakazlar ro‘yxati va tezkor amallar</div>
          </div>
          ...
          <table className="min-w-[1100px] w-full text-sm">
            <thead className="bg-slate-50">
              <tr className="text-slate-600">
                ...
                {columns.code && <th className="p-3 text-left">{sortBtn("createdAt", "Kod")}</th>}
                {columns.barcode && <th className="p-3 text-left">Barcode</th>}
                {columns.status && <th className="p-3 text-left">{sortBtn("status", "Status")}</th>}
                {columns.sla && <th className="p-3 text-left">SLA</th>}

                {columns.sender && <th className={`p-3 text-left ${HIDE_ON_SMALL}`}>Yuboruvchi</th>}
                {columns.recipient && <th className={`p-3 text-left ${HIDE_ON_SMALL}`}>Qabul qiluvchi</th>}

                {columns.city && <th className="p-3 text-left">Shahar</th>}
                {columns.total && <th className="p-3 text-right">{sortBtn("total", "Jami")}</th>}
            ...
            <tbody>
              {orders.map((o) => (
                <tr key={o.id} className="border-t border-slate-200 hover:bg-slate-50/60">
                  {columns.select && (
                    <td className="p-3">
                      <input
                        type="checkbox"
                  ...
                  {columns.sla && (
                    <td className="p-3">
                      <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs ring-1 ${SLA_BADGE[o.slaRisk]}`}>
                        {o.slaRisk === "low" ? "Past" : o.slaRisk === "medium" ? "O‘rtacha" : "Yuqori"}
                      </span>
                    </td>
                  )}
                  ...
                  {columns.sender && (
                    <td className={`p-3 ${HIDE_ON_SMALL}`}>
                      <div className="font-medium text-slate-900 truncate max-w-[260px]">{o.sender.name}</div>
                  ...
                  {columns.recipient && (
                    <td className={`p-3 ${HIDE_ON_SMALL}`}>
                      <div className="font-medium text-slate-900 truncate max-w-[260px]">{o.recipient.name}</div>
                      <div className="text-xs text-slate-500 truncate max-w-[260px]">
                  ...
                  {columns.actions && (
                    <td className="p-3 text-right">
                      <div className="inline-flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => onOpenDetails(o.id)} aria-label="Details">
                          <span className="inline-flex items-center">
                            <Eye className="h-4 w-4" />
                          </span>
                        </Button>

                        <Button size="sm" variant="secondary" onClick={() => onOpenEdit(o.id)} aria-label="Edit">
                          <span className="inline-flex items-center">
      ...
      {/* ✅ MODAL (hozircha skeleton bo‘lishi mumkin) */}
